...
// create a function to reset divs for new data
function resetData() {
...
function plotCharts(id) {
    ...
    d3.json("data/samples.json").then((data => {
        ...
        // filter the samples for user input
        ...
        // plot the bubble chat to the coresponding div
        Plotly.newPlot('bubble', dataBub, layoutBub);


        // for washing frequency = null, replace with zero
        if (wfreq == null) {
            wfreq = 0;
        }
...
// create a function where the ID is the parameter when user input changes dropdown menu option
function optionChanged(id) {

    // reset the data
    resetData();

    // plot the charts for id
    plotCharts(id);
